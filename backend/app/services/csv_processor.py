...
async def clean_and_format_html_async(title: str, raw_description: str, sem: asyncio.Semaphore, tone: str) -> str:
    async with sem:
        ...
        # Ключи строго из фронтенда, инструкции - из "живого" промпта
        style_profiles = {
        ...
        # Если фронтенд прислал что-то странное, берем Neutral
        profile = style_profiles.get(tone, style_profiles["Neutral & Professional"])
...
async def process_all_descriptions(df: pd.DataFrame, file_id: str, merchant_id: str, tone: str):
    ...
    async def task_wrapper(index, title, desc, t):
        cache_key = generate_cache_key(merchant_id, title, desc, t)
        if cache_key in cache:
            return index, cache[cache_key]
        clean_html = await clean_and_format_html_async(title, desc, sem, t)
        cache[cache_key] = clean_html
        return index, clean_html

    tasks = [task_wrapper(index, row["Title"], row.get("Body (HTML)", ""), tone) for index, row in df.iterrows()]
...
def process_csv_file(input_path: str, output_path: str, file_id: str, merchant_id: str, tone: str="Neutral & Professional"):
    try:
        ...
        # Бронебойное сохранение с кавычками
        new_df.to_csv(
            output_path, 
            index=False, 
            encoding="utf-8-sig",
